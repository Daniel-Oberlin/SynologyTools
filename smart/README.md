...
- If no `/dev/sata*` entries are detected, the script exits with code `3`.
...
- Alerts are triggered on increases versus the previous run.
...
- `0`: Completed without execution errors, or completed with alerts when `--alert-error` is not used
